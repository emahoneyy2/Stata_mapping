clear
cd "Q:\DATA\AISU\Elizabeth\Korea\boxplots\bank_specific\WOORI"
...
capture egen WOORI=mean(`x'), by(time num)
replace WOORI=. if num!=4
}
rename WOORI x_WOORI
sort t type
...
line WOORI date, lcolor(yellow) lwidth(.6) xsize(12) ysize(3) plotregion(fcolor(white)) graphregion(fcolor(white)) legend(order(1 "Nationwide" 2 "Regional" 3 "Specialized" 10 "WOORI"))
...
line WOORI date, lcolor(yellow) lwidth(.6) xsize(12) ysize(3) plotregion(fcolor(white)) graphregion(fcolor(white)) legend(order(1 "Nationwide" 2 "Regional" 10 "WOORI"))
