clear
cd "Q:\DATA\AISU\Elizabeth\Korea\boxplots\bank_specific\PUSAN"
...
capture egen PUSAN=mean(`x'), by(time num)
replace PUSAN=. if num!=9
}
rename PUSAN x_PUSAN
sort t type
...
line PUSAN date, lcolor(yellow) lwidth(.6) xsize(12) ysize(3) plotregion(fcolor(white)) graphregion(fcolor(white)) legend(order(1 "Nationwide" 2 "Regional" 3 "Specialized" 10 "PUSAN"))
...
line PUSAN date, lcolor(yellow) lwidth(.6) xsize(12) ysize(3) plotregion(fcolor(white)) graphregion(fcolor(white)) legend(order(1 "Nationwide" 2 "Regional" 10 "PUSAN"))
